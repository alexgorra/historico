...
class MapRenderer:
    ...
    def _generate_map(self):
        ...
        for col in range(cols):
            ...
            self.wall_rects.append(pygame.Rect(x, (wall_row_bottom * TILE_HEIGHT) + TILE_HEIGHT - WALL_HEIGHT, TILE_WIDTH, WALL_HEIGHT))
        
        for row in range(rows):
            y = row * TILE_HEIGHT
            self.wall_tiles.append((0, y, 'l_wall'))
            self.wall_tiles.append((wall_col_right * TILE_WIDTH, y, 'r_wall'))
            self.wall_rects.append(pygame.Rect(0, y, WALL_HEIGHT, TILE_HEIGHT))
            self.wall_rects.append(pygame.Rect((wall_col_right * TILE_WIDTH) + TILE_WIDTH - WALL_HEIGHT, y, WALL_HEIGHT, TILE_HEIGHT))
    ...
    def render(self, surface, camera_x, camera_y):
        ...
        end_col = min(len(self.floor_tiles) // ((WORLD_HEIGHT // TILE_HEIGHT) + 1), int((camera_x + screen_width) // TILE_WIDTH) + 1)
        start_row = max(0, int(camera_y // TILE_HEIGHT))
        end_row = min((WORLD_HEIGHT // TILE_HEIGHT) + 1, int((camera_y + screen_height) // TILE_HEIGHT) + 1)
        
        floor_tile = self.tiles.get('floor')
        if floor_tile and floor_tile.surface:
            for row in range(start_row, end_row):
                for col in range(start_col, end_col):
                    ...
                    surface.blit(floor_tile.surface, (screen_x, screen_y))
        
        for x, y, tile_name in self.wall_tiles:
            screen_x = int(x - camera_x)
            screen_y = int(y - camera_y)
            
            if -TILE_WIDTH <= screen_x <= screen_width and -TILE_HEIGHT <= screen_y <= screen_height:
