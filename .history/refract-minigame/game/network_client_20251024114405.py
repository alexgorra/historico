...
class NetworkClient:
    ...
    def send_message(self, message):
        ...
        if self.connected and self.socket:
            try:
                ...
                print(f"Connection lost while sending message: {e}")
                self.connected = False
                if self.on_disconnect:
                    self.on_disconnect()
            except Exception as e:
    ...
    def _handle_server_message(self, message):
        ...
        try:
            ...
            elif message.startswith(f"{MSG_DAMAGE}:"):
    ...
    def _handle_update_message(self, message):
        ...
        try:
            ...
            print(f"Error parsing UPDATE message '{message}': {e}")
    ...
    def _handle_new_player_message(self, message):
        ...
        try:
            parts = message.split(":")
            if len(parts) >= 5:
                pid, x, y, color = parts[1], int(parts[2]), int(parts[3]), parts[4]
                if pid != self.player_id:
                    self.other_players[pid] = {
                        'x': x,
                        'y': y,
            ...
            print(f"Error parsing NEW_PLAYER message '{message}': {e}")
    ...
    def _handle_projectile_update_message(self, message):
        ...
        try:
            ...
            print(f"Error parsing PROJECTILE_UPDATE message '{message}': {e}")
    ...
    def _handle_damage_message(self, message):
        """Handle damage message from server."""
        # Format: DAMAGE:playerId:currentHp:maxHp
        try:
            parts = message.split(":")
            if len(parts) >= 4:
                player_id = parts[1]
                current_hp = int(parts[2])
                max_hp = int(parts[3])
                if self.on_damage:
                    self.on_damage(player_id, current_hp, max_hp)
        except (ValueError, IndexError) as e:
            print(f"Error parsing DAMAGE message '{message}': {e}")
    
    def _handle_respawn_message(self, message):
        """Handle respawn message from server."""
        # Format: RESPAWN:playerId:x:y
        try:
            parts = message.split(":")
            if len(parts) >= 4:
                player_id = parts[1]
                ...
                y = int(parts[3])
                if self.on_respawn:
                    self.on_respawn(player_id, x, y)
        except (ValueError, IndexError) as e:
            print(f"Error parsing RESPAWN message '{message}': {e}")
