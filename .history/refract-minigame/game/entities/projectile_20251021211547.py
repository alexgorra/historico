...
class ProjectileRendererComponent(Component):
    ...
    def update(self, dt):
        ...
        if self.owner:
            self.trail_positions.append((self.owner.x, self.owner.y))
    ...
    def render(self, surface, camera_x, camera_y):
        ...
        for i, (trail_x, trail_y) in enumerate(self.trail_positions[:-1]):
            ...
            trail_screen_x = int(trail_x - camera_x)
            trail_screen_y = int(trail_y - camera_y)
            trail_size = int(self.visual_size * 0.5 * (i + 1) / len(self.trail_positions))
            ...
            surface.blit(trail_surface, (trail_screen_x - trail_size, trail_screen_y - trail_size))
        ...
        if self.projectile_animation:
            ...
            if current_frame:
                ...
                glow_size = int(self.visual_size * 1.2)
                glow_surface = pygame.Surface((glow_size * 2, glow_size * 2), pygame.SRCALPHA)
                pygame.draw.circle(glow_surface, (*self.color[:3], 60), (glow_size, glow_size), glow_size)
                surface.blit(glow_surface, (screen_x - glow_size, screen_y - glow_size), special_flags=pygame.BLEND_RGBA_ADD)
                return
