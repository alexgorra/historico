...
Maintains same gameplay as original while using modular systems.
"""
import pygame
...
from systems.render_system import AnimationComponent, RendererComponent
from systems.collision_system import CollisionComponent
from entities.projectile import ProjectileRendererComponent, ProjectileMovementComponent
...
class RefactoredGameClient:
    ...
    def handle_input(self):
        ...
        # Input component handles movement automatically in its update
        # We just need to check for shooting and send network updates
        
        # Check if player moved
        old_x, old_y = player.x, player.y
        
        # Update will be done in game_state.update(), but we check if position changed
        movement = player.get_component(MovementComponent)
        if movement and (movement.velocity_x != 0 or movement.velocity_y != 0):
            # Position will change, send update after state update
            pass
        
        # Handle shooting with immediate projectile spawn
        ...
        if should_shoot:
            ...
            self.network_client.send_shoot(
                center_x,
                center_y,
                direction_x,
                direction_y
    ...
    def update(self, dt):
        """Update game state."""
        # Update all game objects (components will update automatically)
        self.game_state.update(dt)
        
        # Update animations for local player with key state
        ...
        # Resolve player-vs-player collisions (prevent phasing through)
        # Simple resolution: if overlapping, push local player back to last non-overlapping position
        if self.game_state.player_object:
            ...
            if local_col:
                ...
                for other in self.game_state.other_players.values():
                    if not other or not other.active or other is local_player:
                        continue
                    other_col = other.get_component(CollisionComponent)
                    if not other_col:
                        continue
                    if lp_rect.colliderect(other_col.get_rect()):
                        # Push back along smallest axis
                        overlap = lp_rect.clip(other_col.get_rect())
                        if overlap.width < overlap.height:
                            # Horizontal resolution
                            if lp_rect.centerx < other_col.get_rect().centerx:
                                # Move left
                                local_player.x -= overlap.width
                            else:
                                # Move right
                                local_player.x += overlap.width
                        else:
                            # Vertical resolution
                            if lp_rect.centery < other_col.get_rect().centery:
                                # Move up
                                local_player.y -= overlap.height
                            else:
                                # Move down
                                local_player.y += overlap.height
    ...
    def render(self):
        ...
        # Get camera position from local player
        ...
        for obj in self.game_state.game_objects:
            ...
            renderer = obj.get_component(RendererComponent)
            if renderer:
                renderer.render(self.screen, camera_x, camera_y)
        
        # Draw debug hitboxes for all entities (if enabled)
        for obj in self.game_state.game_objects:
            if not obj.active:
                continue
